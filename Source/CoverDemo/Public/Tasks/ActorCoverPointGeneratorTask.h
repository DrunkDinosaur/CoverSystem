...
protected:
	...
	// Expand the bounding box of the object by this amount * ScanGridUnit. It's for when the navmesh around the
	// object would exceed the object's bounds by too much, resulting in no cover points being projected onto it.
	float BoundingBoxExpansion;

	// Density of the scan grid (lower number -> more traces); Guideline: should be a bit
	// less than the capsule radius of the smallest unit capable of getting into cover,
	...
	// Not counting bunnies. Bunnies are useless.
	float SmallestAgentHeight;

	// Whether to generate cover points per UStaticMeshComponent found in Owner or around
	// all the colliding components inside the owner's bounding box. Set to true
	// if owner's bounding box would likely intersect with other actors in-game.
	...
	// Gets the nearest ground point to Location that's in one grid unit's range or less. Returns false if ground
	// point was too far, i.e. more than a grid unit away. Does not use the navmesh.
	...
	// Find & store cover points in the game state.
	// Calls GenerateCoverInBounds() either once when bGeneratePerStaticMesh == false or multiple times when bGeneratePerStaticMesh == true
	void DoWork();
